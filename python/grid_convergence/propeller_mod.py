...
class propeller:
	...
	def first_guess(self):
		...
		#~ using bet as an initial guess
		import blade_mod as bm
		gamma = bm.blade_element_theory(self.J, self.nb)
		self.gamma = np.append(gamma,gamma)
		print(self.gamma)
		input()
	...
	def calc_V(self):
		...
		for i in range(self.cp_total):
			...
			if self.aoa[i] < 0.:
				print('Angle of attack is {} degrees at control point {}'.format(self.aoa[i]*180./np.pi, i))
	...
	def calc_gamma(self, tol, max_it):
		...
		e = max(abs(self.R))
		en = e
		g = open('initial_guess_test.txt','a')
		g.write('\n\nBegin Newton\'s solver\n\n')
		g.write('Iteration = {:<3d}  Max residual = {:6.1e}\n'.format(i,e))
		print('\n\nBegin Newton\'s solver\n\n')
		...
		while e > tol:
			...
			if en > eo:
				...
				if temp <= 1.e-10:
					f = open('error.txt', 'a')
					f.write('Advance Ratio: {:3.1f}, nodes: {}, pitch iteration: {}\nFailure due to relaxation factor which is {:5.3f}, Max residual on exit is: {:6.1e}, Approximate error from pitch solver is {:6.1e}\n\n'.format(self.J,self.nb,self.count,temp,e,self.ea))
					f.close()
					break
			...
			if i >= max_it:
				...
				f = open('error.txt', 'a')
				f.write('Advance Ratio: {:3.1f}, nodes: {}, pitch iteration: {}\nFailure due to max iterations which is {:9d}, Max residual on exit is {:6.1e}, Approximate error from pitch solver is {:6.1e}\n\n'.format(self.J,self.nb,self.count,i,e,self.ea))
				f.close()
				break
		g.close()
	...
	def global_iterative_pitch_solver(self, tol1, tol2, maxit):
		...
		while ea > tol1:
			...
			g.write('\n\n\nPitch Iteration {}\n\n'.format(self.count))
			g.close()
			print('\n\n\nPitch Iteration {}'.format(self.count))
			...
			if temp1 <= 0.:
				f = open('error.txt', 'a')
				f.write('Advance Ratio: {:3.1f}, nodes: {}, pitch iteration: {}\nFailure due to pitch relaxation factor which is {:5.3f}, Approximate error for the global pitch solver is: {:6.1e}\n\n'.format(self.J,self.nb,self.count,temp1,ea))
				f.close()
				break
			
			self.b[:,:] += temp1 * (temp - self.b[0,0])

...
! dimensional
!***************************************************************************
subroutine calc_nu(nu, a1, a2, b1, b2, rl, control, n, m, p, phi, int_type, ij)
	implicit none
	integer, parameter :: wp = selected_real_kind(p=8)
	real(wp), intent(in) :: a1, a2, b1, b2, n, m, p, phi, rl
...
!f2py intent(in,out) :: nu
	...
	call integrate(v, a1, b1, rl, control, n, m, p, phi, int_type)
	!*******************************************************************************************
	call integrate(w, a2, b2, rl, control, n, m, p, phi, int_type)
	!************************************************************************************
	if (.not. ij) then
		j1(0) = a1 * cos(phi)
		j1(1) = a1 * sin(phi)

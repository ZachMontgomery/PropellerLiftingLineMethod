...
import contracting_helix as ch
...
class propeller:
	...
	def __init__(self, filename):
		...
		for i in range(self.cp_total):
			self.dA[i] = (self.c[i,0] + self.c[i,1]) / 2. * (ch.vec_mag(self.node[i,1,:]) - ch.vec_mag(self.node[i,0,:]))
	...
	def calc_R(self):
		...
		for i in range(self.cp_total):
			self.R[i] = 2.*self.gamma[i]*ch.vec_mag(np.cross(self.V[i,:],self.dl[i,:]))-ch.vec_mag(self.V[i,:])**2.*self.Cl[i]*self.dA[i]
	...
	def calc_Jacobian(self):
		...
		for i in range(self.cp_total):
			...
			for j in range(self.cp_total):
				term1 = 2. * self.gamma[i] * np.dot(w[i,:],np.cross(self.nu[i,j,:],self.dl[i,:])) / ch.vec_mag(w[i,:])
				
				term2 = 2. * self.Cl[i] * np.dot(self.V[i,:],self.nu[i,j,:]) * self.dA[i]
				
				term3 = ch.vec_mag(self.V[i,:]) ** 2. * self.Cla[i] * (Va[i] * np.dot(self.nu[i,j,:],self.un[i,:]) - Vn[i] * np.dot(self.nu[i,j,:],self.ua[i,:])) / (Vn[i] ** 2. + Va[i] ** 2.) * self.dA[i]
				
				term4 = 2. * ch.vec_mag(w[i,:])
				if i != j:
	...
	def calc_forces_and_moments(self):
		...
		for i in range(self.cp_total):
			...
			self.Cm[i] = -.1
			self.dM[i,:] = -1./8.*self.rho*ch.vec_mag(self.V[i,:])**2.*self.Cm[i]*(self.c[i,0]+self.c[i,1])**2.*(ch.vec_mag(self.node[i,1,:])-ch.vec_mag(self.node[i,0,:]))*self.us[i,:]
	...
	def calc_coef(self):
		self.CT = -self.total_force[2] / abs(self.total_force[2]) * ch.vec_mag(self.total_force)  / self.rho / (2.*self.rp)**4. / (self.w/2./np.pi)**2.
		self.CL = -self.rl * self.total_moment[2] / abs(self.total_moment[2]) * ch.vec_mag(self.total_moment) / self.rho / (2.*self.rp)**5. / (self.w/2./np.pi)**2.
	...
	def renew_nu(self):
		...
		for j in range(self.cp_total):
			...
			for i in range(self.cp_total):
				...
				#self.nu[i,j,:] = ch.nu(a1,a2,self.b[j,0],self.b[j,1],self.n,self.c1,self.c2,phi,self.rl,x,y,z,self.m,self.p,i,j)
	...
	def slipstream_contraction_solver(self, tol1, tol2, maxit):
		
		count = 1
		ea = 1.
		
		def my_func(z, a, c1, c2):
			return a * (1. + (c2 - 1.) * z / np.sqrt(z**2. + c1**2.))
		
		while ea > tol1:
			
			print('\n\n\nSolver Iteration {}'.format(count))
			print('Current global pitch is {}\n\n'.format(self.b[0,0]))
			#input()
			self.pll(tol2, maxit)
			self.Ct_guess = -1.
	...
	def iterate_slipstream_contraction_variables(self):
